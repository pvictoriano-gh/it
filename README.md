...
A lightweight, dependency-free portfolio site for **Patrick Victoriano**, an IT
support and systems professional. Built with plain HTML, CSS, and a little
vanilla JavaScript, and designed to be published for free via **GitHub Pages**.

The page is a single scroll with these sections: **About**, **Skills & Tools**
(grouped by IT domain — service management, operating systems, directory/server
roles, networking & virtualisation, monitoring, scripting/cloud/AI, databases,
hardware), **Experience**, **Highlights**, **Certifications**, **Education**, and
a **Contact** form.
...
```text
portfolio-site/
├── index.html             # Page content, structure & inline JS (theme, terminal mode, scroll, contact form)
├── 404.html               # Custom terminal themed not-found page
├── css/
│   ├── style.css          # Base styling (edit the :root variables to re-theme)
│   └── terminal-mode.css  # "Terminal mode" re-skin, scoped under <html data-mode="terminal">
...
A **"Turn on terminal mode"** button in the footer flips the whole page into a
system/terminal inspired theme (indigo accent, monospace shell-command cues, a
status readout block, and "✓ ok" pills). It sets `<html data-mode="terminal">`,
which `terminal-mode.css` keys off, and the choice is remembered via
`localStorage`. Click
**"Turn off terminal mode"** to return to the standard view. Terminal mode has
its own light and dark palettes, so the navbar 🌙 / ☀️ toggle works there too.
...
2. Assets live in the `assets/` folder:
   ...
   - `pvictoriano-resume-it.pdf` — the résumé; the **Download résumé** button
     (hero) and the **Résumé** link (contact) open it in a new tab
...
3. Want different colors? Edit the variables under `:root` (dark theme) and
   `[data-theme="light"]` (light theme) at the top of `css/style.css`. The
   terminal-mode palette lives in `css/terminal-mode.css`.
...
`<head>` to avoid a flash. New viewers also start in **standard** mode.
